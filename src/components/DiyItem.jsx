...
const DiyItem = () => {
  ...
  useEffect(() => {
    axios.get(`https://whippedbehind.hyee34.site/get_product`).then((response) => {
    ...
    if (existingItem) {
        ...
        if (shouldUpdate) {
            try {
                ...
                await axios.post(
                    `https://whippedbehind.hyee34.site/update_diy_quantity/${userId}`,
                    {
        ...
        try {
            await axios.post(`https://whippedbehind.hyee34.site/add_Diyitem/${userId}`, cartItem, { 
                headers: { 'Content-Type': 'application/json' }
            });
            alert('장바구니에 추가되었습니다!');
